...
export function AdminFlowView({ data, dataPage, view, tab = "overview", range }: { data: TtgDashboardData; dataPage?: SupabaseDataPage; view: string; tab?: string; range: DashboardRange }) {
    ...
    return <>
      ...
      <div className="ttg-af-grid">
        ...
        <Panel title="Appointment mix" note={`${integer.format(appointments)} appointments in ${range.label}`}>
          <InteractiveDonut href={appointmentsHref} data={[
  ...
  if (view === "team") {
    ...
    return <><Tabs active={tab} range={range} tabs={tabs} view={view} /><Cards items={[{ label: "Active practitioners", value: integer.format(active), detail: range.label }, { label: "Avg. invoiced / practitioner", value: active ? cad.format(invoiced / active) : "—", detail: "Jane Sales" }, { label: "Team completion", value: pct(completionRate), detail: `${integer.format(completed)} completed` }, { label: "Missed appointments", value: integer.format(cancelled + noShows), detail: "Cancelled + no-show" }]} />
      ...
      ]} /></Panel> : <Panel title={tab === "cancellations" ? "Missed appointments by practitioner" : tab === "efficiency" ? "Completion rate by practitioner" : "Invoiced revenue by practitioner"} note="Hover for exact values"><InteractiveBarChart currency={chartKey === "invoiced"} data={chartRows} horizontal href={tab === "cancellations" || tab === "efficiency" ? appointmentsHref : salesHref} valueKey={chartKey} /></Panel>}
    </>;
  }
  ...
  if (view === "marketing") {
    ...
    return <><Tabs active={marketingTab} range={range} tabs={tabs} view={view} />
      ...
      {marketingTab === "trends" && <><Cards items={[
        ...
        { label: "New Clients", value: integer.format(newPatients), detail: "Jane first visits" },
      ]} /><div className="ttg-af-grid"><Panel title="Spend vs. New Clients" note="Weekly acquisition activity"><InteractiveTrendChart data={marketingTrend} href={appointmentsHref} series={[{ key: "newPatients", label: "New Clients", color: "#2f86ba" }, { key: "spend", label: "Spend", color: "#61b08b", type: "line" }]} /></Panel><Panel title="Gross ROAS Over Time" note="Appears when spend overlaps Jane revenue"><InteractiveLineChart data={marketingTrend.map((row) => ({ ...row, roas: 0 }))} href={appointmentsHref} series={[{ key: "roas", label: "Gross ROAS", color: "#2f86ba" }]} /></Panel></div><Panel title="New Client CAC Over Time" note="Weekly campaign spend divided by new clients"><InteractiveLineChart currency data={marketingTrend.map((row) => ({ ...row, cac: 0 }))} href={appointmentsHref} series={[{ key: "cac", label: "New Client CAC", color: "#e2a256" }]} /></Panel></>}
    </>;
  }
