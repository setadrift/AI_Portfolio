...
import { updateOffer } from "@/lib/portal/admin/acquisition-db";
...
export async function PATCH(
  ...
  const body = (await request
    .json()
    .catch(() => null)) as Partial<ConsultingOfferRecord> | null;
  if (!body)
  ...
  return NextResponse.json(
    result.ok ? { ok: true, offer: result.data } : { error: result.error },
